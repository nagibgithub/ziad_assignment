const categoryFetch = () => {
    fetch('https://openapi.programming-hero.com/api/videos/categories').then(res => res.json()).then(data => {
...
const navCategory = data => {
    data.forEach(category => {
        document.getElementById('nav-category').innerHTML += `<div class="mx-2"><button onclick={allDataFetch(${category.category_id})} id="${category.category_id}" class="category-btn py-2 px-4 bg-[#2525258c] text-white rounded font-semibold shadow hover:bg-[#FF1F3D] hover:text-white active:bg-[#ff6d80]">${category.category}</button></div>`
    });
};

const allDataFetch = (categoryId) => {
    loadingSping(true);
    fetch(`https://openapi.programming-hero.com/api/videos/category/${categoryId}`).then(res => res.json()).then(data => {
        allVideo(data.data);
        loadingSping(false);
    });
};
...
console.log(video.others.posted_date);
            ...
            <div class="flex flex-col items-center">
                  <div class="bg-gray-100 px-5 py-10 rounded-lg">
                    <div class="relative">
                        ...
                        <div class="">
                            <h6 id="a-${i}" class="absolute bg-black p-1 rounded bottom-1 right-1 text-white text-xs"></h6>
                        </div>
                    </div>
            ...
            if (video.others.posted_date !== "") {
                const postedDateInMili = parseInt(video.others.posted_date) * 1000;
                const currentDate = new Date();

                const timeDifferenceInMilliseconds = currentDate - postedDateInMili;

                const hoursAgo = Math.floor(timeDifferenceInMilliseconds / (10000000 * 60 * 60));
                const minutesAgo = Math.floor((timeDifferenceInMilliseconds % (1000 * 60 * 60)) / (1000 * 60));

                if (hoursAgo > 0) {
                    document.getElementById(`a-${i}`).innerText = `${hoursAgo}hrs ${minutesAgo}min ago`;
                } else {
                    document.getElementById(`a-${i}`).innerText = `${minutesAgo}minutes ago`;
                }
            };
        });
    } else {
        document.getElementById('video-container').innerHTML = `<div class="flex flex-col justify-center col-span-3 items-center h-full py-7"><img src="./Icon.png" alt=""><h1 class="text-[32px] font-bold text-[#171717] text-center mt-5">Oops!! Sorry, There is no <br> content here</h1></div>`
    };
}

const loadingSping = loadingStatus => {
    if (loadingStatus) {
...
const callFunctionOnLoad = () => {
    categoryFetch();
    allDataFetch(1000, false);
};

const extractNumericViews = views => parseFloat(views.replace("K", "")) * 1000;

const sortByViews = data => {
    // Sort the data array by views in descending order
    data.sort((a, b) => {
        const viewsA = extractNumericViews(a.others.views);
        const viewsB = extractNumericViews(b.others.views);
